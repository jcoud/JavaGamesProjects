...
class Logic {
    ...
    static void init() {
        name_list.add(ShapeNames.O); shape_list.add(o);
        name_list.add(ShapeNames.T); shape_list.add(T);
        name_list.add(ShapeNames.L); shape_list.add(L);
        name_list.add(ShapeNames.J); shape_list.add(J);
        name_list.add(ShapeNames.S); shape_list.add(S);
        name_list.add(ShapeNames.Z); shape_list.add(Z);
        name_list.add(ShapeNames.I); shape_list.add(I);
    ...
    static void newGame() {
        Reference.field = new int[canvasMetrix.height / box + 2][canvasMetrix.width / box + 4];
        setBorder();
        gameOver = false;
        pause = false;
        score = 0;
...
//        delayInc = maxDelay;
    ...
    static void removeLine() {
        ...
        for (int i = field.length - 3; i >= 2; i--) {
            int line0 = 1;
            for (int j = 2; j <= field[0].length - 3; j++) {
                line0 *= field[i][j];
            }
            if (line0 > 1) {
                line[i] = 1;
...
//        if (delayInc <= maxSpeed) {
//            delayInc = maxDelay - (level - 1) * 24;
//        }
//        System.out.printf("d:%d\r", delay);
        field = field_edit;
    }
    ...
    static boolean notCollideWithDir(EnumDir checkDir) {
        int[][] array = current_rotated_shape.clone();
        int x = dx, y = dy;
        if (checkDir == EnumDir.DOWN) y++;
        if (checkDir == EnumDir.LEFT) x--;
        if (checkDir == EnumDir.RIGHT) x++;
